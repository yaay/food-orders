...
if(isset($_POST['update_product'])){
   ...
   $update_product = $db->prepare("UPDATE `restaurant`.`products` SET name = ?, price = ? WHERE id = ?");
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $db->prepare("UPDATE `restaurant`.`products` SET image = ? WHERE id = ?");
...
<section class="update-product">
   ...
   <?php
      ...
      $select_products = $db->prepare("SELECT * FROM `restaurant`.`products` WHERE id = ?");
      $select_products->execute([$update_id]);
      if($select_products->rowCount() > 0){
         while($fetch_products = $select_products->fetch(PDO::FETCH_ASSOC)){ 
   ?>
   <form action="" enctype="multipart/form-data" method="post">
      <input name ="id" type="hidden" name="pid" value="<?= $fetch_products['ID']; ?>">
      ...
      <div class="flex-btn">
         ...
         <a href="admin_show_products.php" class="option-btn">Go back</a>
      </div>
   </form>
